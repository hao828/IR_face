...
def raw2Celsius(raw):
    ...
    slope = 0.001   
    return slope*raw+ambientTemperature-77;
...
def xbee_process(Max_TEXT,XBee_MAC):  #xbee_process(TEXT,XBee_MAC)
    try:
        Max_TEXT = str(Max_TEXT)
        device = XBeeDevice('/dev/ttyS0', 9600)
        device.open(force_settings=True)                     
        send_str = chr(0x02) + XBee_MAC + ',' + '1' + ',' + Max_TEXT + chr(0x03) + chr(0x13) + chr(0x10) +'\r\n'
        device.send_data_broadcast(send_str)        
